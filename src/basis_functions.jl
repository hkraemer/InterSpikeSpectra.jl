...
"""
    ...
    Generate a matrix `basis` with all possible `sum(1:ceil(N/2))+1` spike-basis functions.
...
function generate_basis_functions(N::Int)
    ...
    basis[end,:] = create_single_basis_function(N, Int(ceil(N/2)+1))[1,:]
    return basis
end
